...
5. **Full scan path** (still single-bw) — **In progress / analysis.** Large image traffic exists in **`ls40-single-bw.pcapng`** immediately after duplicate `GET_WINDOW` traffic (~frame **2399+**): `READ(10)` allocations **258048**, **223488**, then many **259200** issues plus a **103680** tail; wire export uses **65508-byte** IN rows per SCSI issue. **`scripts/audit_capture_read_batches.py`** documents allocation vs wire chunk counts. Consolidating tens of MiB into replay `@` blobs and matching **`perform_scan_sequence`** (or Nikon’s ordering) remains before locking **`test_basic_scan_capture.txt`** for full scan.
