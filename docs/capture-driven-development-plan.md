...
| Human/machine-friendly trace | `test_basic_scan_capture.txt` (columns: time, endpoint, length, hex or `@path` for large IN). **290 lines** today. **1–83:** init through MODE_SELECT (replay-locked). **84–87:** extra `TEST_UNIT_READY` before the prescan-aligned window. **88–208:** full `prescan()` USB replay slice (includes post-`START_SCAN` status, poll, image READs, exposure, `GET_WINDOW`). **210–254:** full-scan setup + polling for `perform_scan_sequence()` (TUR, reserve, set_window via MODE_SELECT, 3× LUT upload, start_scan, PROCESSING→READY polling). **256–290:** full-scan image READs (64, 258048, 259200, 103680 bytes) with `@tests/fixtures/scan_image_block*.bin`. |
...
**Full scan image replay:** `tests/test_usb_replay_full_scan_sequence.py::test_full_scan_image_reads_match_capture` — **`perform_scan_sequence()`** + 4× `read_scan_data()` calls match **lines 210–290** (64-byte synthetic, 258048-byte first stripe, 259200-byte main stripe, 103680-byte tail). Large INs use **`@tests/fixtures/scan_image_block{1,3,4}.bin`** rebuilt from `ls40-single-bw.pcapng`.
...
**Last updated (2026-06):** Milestone 5 extended — full scan image replay via `test_usb_replay_full_scan_sequence.py` (lines 210-290, 4 image READs including 258048/259200/103680-byte stripes). Milestone 6 extended — batch scan replay via `test_usb_replay_batch_scan.py` (reserve, 3× READ_CAPACITY, START_SCAN, polling, 4× READ_SCAN_DATA, RELEASE_UNIT). `read_capacity()` accepts `window_id` parameter. `object_position` removed (LS-40 ED rejects). Total: **157 tests**.
## Subagent roles
